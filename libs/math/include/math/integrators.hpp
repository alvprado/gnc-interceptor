...
/// @brief Integration policy: a single first-order explicit (forward) Euler
/// step.
struct EulerStep
{
...
/// @brief Integration policy: a single second-order explicit-trapezoidal (Heun)
/// step.
struct HeunStep
{
                          ...
                          typename StateVec_T::Scalar dt) const
    {
        ...
        // Evaluate each stage state into a concrete vector before calling the
        // model, so the model always receives a plain vector rather than an
        // unevaluated Eigen expression.
        const StateVec_T k1 = f(x, u);
