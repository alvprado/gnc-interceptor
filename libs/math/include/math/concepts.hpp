...
/// @brief An Eigen column vector: a single-column MatrixBase expression.
/// Exposes ::Scalar.
...
/// @brief A callable f(x, u) mapping a state and a control to a state-space
/// vector.
/// @details The lower-level, dimension-agnostic contract the integrators and
/// differentiation policies build on. It fits both a continuous model (ẋ = f(x,
/// u)) and a discrete step (x_{k+1} = f(x, u)); both share the (state,
/// control) -> state signature.
/// @tparam Fn A callable invoked as f(x, u).
/// @tparam StateVec_T The state (column) vector type; f returns something
/// convertible to it.
